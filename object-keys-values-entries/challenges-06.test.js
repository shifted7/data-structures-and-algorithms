...
const hasChildrenValues = (arr, name) => {
    for(let charIndex in arr){
        if(arr[charIndex].name===name){
            let charValues = Object.values(arr[charIndex]);
            let children = charValues[2];
...
const hasChildrenEntries = (arr, name) => {
    for(let charIndex in arr){
        if(arr[charIndex].name===name){
            let charEntries = Object.entries(arr[charIndex]);
            let children = charEntries[2][1];
            if(children.length>0){
                return true;
            }
        }    
...
const totalCharacters = (arr) => {
  ...
  for (let i in arr){
    if(arr[i].name){
        totalCharacters++;
        let charValues = Object.values(arr[i]);
        if (charValues[1]){
            totalCharacters += 1; // spouse
        }
        let children = charValues[2];
        totalCharacters += children.length;
    }
  }
...
const houseSize = (arr) => {
  const sizes = [];
  for (let charIndex in arr){
    let charValueList = Object.values(arr[charIndex]);
    let houseName = charValueList[3];
    let membersNum = 0
    if (charValueList[0]){
       membersNum += 1; // character
       if (charValueList[1]){
           membersNum += 1; // spouse
       }
       if(charValueList[2]){
          membersNum += charValueList[2].length; // # of children
      }
    }
    // console.log(membersNum);
    let house = {'house': houseName,'members': membersNum};
    sizes.push(house);
  }
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  for (let charIndex in arr){
    ...
    let survivorNum = 0
    if (charValueList[0]){
       survivorNum += 1; // character
       if (charValueList[1]){
        let deceased = deceasedSpouses.some(spouse => {
            return spouse === charValueList[1];
        });
        if (!deceased){
            survivorNum += 1; // spouse
        };
       };
       if(charValueList[2]){
        survivorNum += charValueList[2].length; // # of children
      }
    }
    let house = {'house': houseName,'members': survivorNum};
