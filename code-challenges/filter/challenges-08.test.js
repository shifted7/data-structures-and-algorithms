...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(str => {
    for (let forbidVal of forbiddenValues){
      if(str.toString().includes(forbidVal)){
        return false;
      }
    }
    return true;
...
const getStatName = (arr, minBaseStat) => {
  let matchingStats = arr.filter(statObj => statObj.baseStat>minBaseStat);
  return matchingStats.map(statObj=>statObj.stat.name);
}
