...
const count = (target, input) => {
  return input.reduce((totalCount, arr)=>{
    return totalCount + arr.reduce((arrayCount,value)=>{
     if(value===target){
      arrayCount++;
     }
     return arrayCount
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arr=>{
    let filteredArr = arr.filter(value=>value%5===0 && typeof(value)==='number');
    return filteredArr.map(value=>Math.pow(2,value));
  });
};
...
let findShortest = (data) => {
  let resultShortestCharacter = data.reduce((shortestCharacter, character)=>{
    if(Number(character.height) < Number(shortestCharacter.height)){
      return character;
    }
    return shortestCharacter;
  },data[0]);
  return resultShortestCharacter.name;
};
